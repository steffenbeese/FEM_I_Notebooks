...
class BalkenFEM(FEM1D):
    ...
    Zusätzliche Attribute:
        dL (np.ndarray): Verteilte Lasten an den Knoten.
    ...
    def shapeFunction(self, xi):
        ...
        """
        return np.array([2*xi**3 - 3*xi**2 + 1, 
                        (xi**3 - 2*xi**2 + xi), 
                        -2*xi**3 + 3*xi**2, 
                        (xi**3 - xi**2)])
    ...
    def computeMoment(self, n=10):
        ...
        def callback(elID, xi, le, dofe):
            EI = self.eData[elID]["youngsmodulus"] * self.eData[elID]["sma"]
